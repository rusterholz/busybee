...
RSpec.describe Busybee::Client, "#update_job_retries" do
  let(:job_bpmn_path) { File.expand_path("../../fixtures/job_process.bpmn", __dir__) }

  shared_examples "update_job_retries" do
    around do |example|
    ...
    it "updates job retries successfully" do
      with_activated_job_instance("process-order") do |job|
        result = client.update_job_retries(job.key, 5)
    ...
    it "raises error when job key does not exist", :skip_process_instance do
      ...
      expect { client.update_job_retries(non_existent_key, 3) }.to raise_error(Busybee::GRPC::Error) do |error|
  ...
  context "with local Zeebe", :integration do
    ...
    it_behaves_like "update_job_retries"
  end
