...
namespace :gemfile do
  # Only committed lockfiles have to carry every platform, and those are the root
  # one and the demo's. The Appraisal lockfiles are generated per machine (and
  # gitignored), and CI resolves them fresh, so whatever platform it resolves on
  # is the right one there.
  desc "Ensure all platforms are present in the committed lockfiles"
  task :platforms do
